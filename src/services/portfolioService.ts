import BalanceService, { type TokenBalance } from './balanceService';
import PriceService, { type TokenPrice } from './priceService';
...
/**
 * Portfolio Service
 * Combines real balances with live prices to show complete portfolio view
 */
class PortfolioService {
  /**
   * Get complete portfolio with real balances and live prices
   */
  static async getPortfolio(networkId: string = 'mainnet'): Promise<Portfolio> {
    try {
      ...
      console.log(`[PortfolioService] Retrieved ${prices.length} prices`);
  ...
  static async refreshPortfolio(networkId: string = 'mainnet'): Promise<Portfolio> {
    ...
    PriceService.clearCache(); // Clear price cache to get fresh prices
